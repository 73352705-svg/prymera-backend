"""Create bd_core_mobile database and seed data"""
import psycopg2
...
cur.execute("SELECT 1 FROM pg_database WHERE datname = 'bd_core_mobile'")
if not cur.fetchone():
    cur.execute("CREATE DATABASE bd_core_mobile")
    print("Database bd_core_mobile created")
else:
    print("Database bd_core_mobile already exists")
...
if db.query(Agencia).count() == 0:
    ...
    for asesor in [e1, e2]:
        for cliente in [c1, c2, c3, c4]:
            item = CarteraDiaria(id=uuid.uuid4(), asesor_id=asesor.id, cliente_id=cliente.id, agencia_id=aq.id if asesor == e1 else cs.id,
                fecha_asignacion=date.today(), tipo_gestion=random.choice(gestiones), prioridad=random.choice(prioridades),
                score_prioridad=random.randint(0, 100), monto_credito=random.randint(1000, 15000))
            db.add(item)
    db.commit()

    uc = UsuarioCliente(id=uuid.uuid4(), cliente_id=c5.id, username="11111111",     password_hash=hash_password("Prymera2024"), activo=True)
    db.add(uc)
    db.commit()
...
db.close()
    print("\nDone! Credentials:")
    print("  Asesores: 0001/0002/0003/9999 | password: Prymera2024")
